...
# Open dataset.
url = 'http://tds.mercator-ocean.fr/thredds/dodsC/glorys12v1-daily-gridKZ'
...
for i in range(calendar.monthrange(YEAR, MONTH)[1]):
    ...
    # Save dataset.
    dsi.to_netcdf(f'glorys12_kz_{YEAR}_{MONTH:02d}_{i+1:02d}.nc', format = 'NETCDF4')

# Concatenate daily files.
ds = xr.open_mfdataset(f'glorys12_kz_{YEAR}_{MONTH:02d}_*.nc')
ds.to_netcdf(f'glorys12_kz_{YEAR}_{MONTH:02d}.nc', format = 'NETCDF4')

# Delete daily files.
for i in range(calendar.monthrange(YEAR, MONTH)[1]):
    os.remove(f'glorys12_kz_{YEAR}_{MONTH:02d}_{i+1:02d}.nc')
