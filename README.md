...
- **Particle release**: ~546,000 particles/day from slope zone within CCAMLR Statistical Areas
...
- **Production-ready**: Validated against published benchmarks; used for the 32-year production hindcast (1994–2025)
- **Robust error handling**: Three simulation versions (run.py, run_bis.py, run_ter.py) for problematic dates, with `Runs/KRICO_GLORYS12/run_variants.csv` recording which version produced each release date of the production run
...
- **Software**: Parcels v3.1.4 on Python 3.13.7 (the environment used for the production hindcast), plus xarray, geopandas, numpy, mpi4py
- **Data access**: Internet or institutional access to Mercator Ocean OpenDAP servers (for downloading GLORYS12v1)
- **Storage**: ~6 TB for the full 32-year dataset (3 848 release days, compressed NetCDF4)
